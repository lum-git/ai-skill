...
description: >
  ...
  统计今天用户创建的任务 / daily user task creation report. 统计今天由 Operator 或 Admin
  角色用户通过 Paperclip Web UI 人工创建的任务，按创建人分组、组内按公司分组，生成
  Markdown 日报（创建/完成/堵塞/评审中/完成率 + 任务明细），保存为任务评论 + Markdown
  附件，并推送钉钉机器人（自定义 Webhook，关键词 ai）。
...
处理规则：
- 保留 `membershipRole` 为 `operator` 或 `admin` 的用户（不区分是否为 CEO / 团队负责人，角色规则 v4，2026-08-10，PAP-6827：Admin 角色用户人工创建的任务统一纳入统计）；
...
- 绝对不要把 owner / viewer / 系统账号（local-board）创建的任务混进报告。
